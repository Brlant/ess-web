...
<template>
  <div>
    <header class="main-header">
      <div class="container">
        <router-link class="a-link" to='/'>
          <div :class="{'mix-logo':logisticsCentreName}" class="top-logo">
            <div class="logo-img">
              <img :src="logo_pic" @click="activeId=''" class="logo_pic">
            </div>
            <div>
              <div class="main-title">MCC冷链监控</div>
              <el-tooltip :content="logisticsCentreName" effect="dark" placement="bottom">
                <div class="sub-title">
        ...
        <nav class="top-menu">
          <ul>
            <li :class="{'active':activeId === item.meta.moduleId,'hover':hoverItem===item}" :key="menu.path"
                @click.stop.prevent="goTo(item.children.length ? item.children[0]:item)"
                @mouseenter="hoverItem=item"
                @mouseleave="hoverItem={}" v-for="item in menu">
              ...
              <ul class="top-sub-menu" v-if="item.children.length">
                <li :key="child.path" @click.stop.prevent="checkSubMenu(child)" class="top-sub-item"
                    v-for="child in item.children">{{ child.meta.title}}
                </li>
              </ul>
            </li>
          </ul>
    ...
    <div class="sub-menu" v-if="subMenu.length>0">
      <div class="container">
        <a :class="{'active':selectSubmenu(item)}" :href="item.path"
           @click.stop.prevent="goTo(item)"
           class="btn btn-sub-menu"
           v-for="(item,key) in subMenu">{{item.meta.title}}</a>
      </div>
    </div>
  </div>
</template>
