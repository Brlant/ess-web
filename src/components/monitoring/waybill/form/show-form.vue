        ...
        <template slot="content">
            ...
            <div v-else v-loading="loading">
                <div class="form-header-part">
                    ...
                    <div class="content" style="overflow: hidden">
                        <el-row>
                            ...
                            <oms-col :isShow="true" :rowSpan="rowSpan" label="订单号" v-show="formItem.orderNo">
                                <div class="orderNo-class">{{formItem.orderNo}}</div>
                            </oms-col>
                        ...
                        <el-row>
                            <oms-col :isShow="true" :rowSpan="rowSpan" label="启运时间">{{formItem.departTime | minute}}
                            </oms-col>
                            <oms-col :isShow="true" :rowSpan="rowSpan" label="送达时间">{{formItem.arriveTime | minute}}
                            </oms-col>
                        </el-row>
                        <el-row>
                            <!-- <oms-col :isShow="true" :rowSpan="rowSpan" label="运单监控开始时间">{{formItem.createTime | time}} -->
                            <oms-col :isShow="true" :rowSpan="rowSpan" label="运单监控开始时间">{{formItem.orderStartTime | time}}
                            </oms-col>
                            <oms-col :isShow="true" :rowSpan="rowSpan" label="数据截止时间" v-if="handOverList[0]">
                                {{handOverList[0].endTime
                                |
                                minute}}
                            </oms-col>
                        </el-row>
                ...
                <div class="form-header-part">
                    ...
                    <div>
                        <el-form :model="detailForm" class="clearfix" label-width="100px" ref="detailForm"
                                 v-show="showAddFlag">
                            <el-row>
                                <el-col :span="20">
                                    <el-form-item label="设备" style="margin-top: 20px;margin-bottom: 20px">
                                        <el-select :clearable="true" :remote-method="queryDevList"
                                                   @click.native="queryDevList('')"
                                                   filterable
                                                   multiple placeholder="请输入设备名称/编号/编码查询设备"
                                                   popperClass="good-selects" remote v-model="detailForm.devIdList">
                                            <el-option :key="ccsDev.ccsDevId" :label="ccsDev.devName"
                                                       :value="ccsDev.ccsDevId"
                                                       v-for="ccsDev in ccsDevList">
                                                <div style="overflow: hidden">
                                                    <span class="pull-left"
                                                          style="clear: right">{{ccsDev.devName}}</span>
                                                </div>
                                                <div style="overflow: hidden">
                          <span class="select-other-info pull-left">
                            ...
                            <span v-show="ccsDev.devNo">设备编号:</span>{{ccsDev.devNo}}
                                ...
                                <el-col :span="4" style="margin-top: 20px;margin-bottom: 20px;padding-left: 40px">
                                    <el-button :disabled="doing" @click="onSubmit('detailForm')" type="primary">保存
                                    </el-button>
                                </el-col>
                            </el-row>
                        </el-form>
                                ...
                                <div :key="index" class="order-list-item no-pointer order-list-item-bg"
                                    ...
                                    <el-row>
                                        ...
                                        <el-col :span="6" v-show="!formItem.arriveTime">
                                            <el-tooltip :content="showRecordDate(item.recordDate)" effect="dark"
                                                        placement="top">
                ...
                <div class="form-header-part">
                    ...
                    <div class="content">
                        <div v-if="plateNumbers.length">
                            <vehicle-form :devInfo="formItem" :carDevList="carDevList" :plateNumbers="plateNumbers"
                                          :queryVhDevInfo="queryAllVhDevInfo"
    ...
    export default {
        ...
        methods: {
            ...
            queryDetail() {
                ...
                this.isLoadedFirst = false;
                this.loading = true;
                this.$http.get(`/waybill-car-dev-relation/${this.formItem.ccsOrderId}`).then(res => {
                    this.tempList = res.data.orderCcsDevDTOList || [];
                    this.carDevList = res.data.carCcsDevDTOList || [];
                    this.loading = false;
            ...
            queryHandOverList() {
                ...
                this.$http.get('/mcc-data/handover-data/info', {params}).then(res => {
                    if (res.data[0] && res.data[0].dataDtoList) {
                        res.data[0].dataDtoList.forEach(i => {
