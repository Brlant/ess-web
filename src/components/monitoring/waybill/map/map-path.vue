  ...
  <div>
    <h3 v-show="vid==='waybillMap'">
      ...
      <span @click="showBigMap(vhList)" class="des-btn">
            <a @click.prevent="" class="btn-circle" href="#">
                 <i class="el-icon-zoom-in"></i></a>查看大图
      </span>
        <span @click="exportTrajectory" class="des-btn">
            <a @click.prevent="" class="btn-circle" href="#">
                 <i class="el-icon-download"></i></a>导出Excel
      </span>
    </h3>

    <!--<div v-show="!isHasPath" class="empty-info mini">暂无轨迹信息</div>-->
    <div style="position: relative">
      <el-amap :amap-manager="amapManager"  :style="mapStyle"
                :zoom="10" class="map-path" ref="pathMap">
      </el-amap>
      <el-checkbox border class="map-btn" label="1" size="mini" v-model="isShowTemp" v-show="vid!=='waybillMap'">温度
      </el-checkbox>
    </div>
  </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            createInfoWindow() {
                ...
                this.infoWindow = new AMap.InfoWindow({offset: new AMap.Pixel(0, -2)});
            },
            openInfoWindow(item) {
                this.infoWindow.setContent(this.getContent(item));
                this.infoWindow.open(this.amapManager._map, item.lnglat);
            },
            getContent(item) {
                let content = item.devTempList.map((t, index) => {
                    ...
                    if (!tempData) return '';
                    return `<div>${tempData}℃ | ${t.name}</div>`;
                }).join('');
                return `<div class="point-content">
                         <div><div class="point-content__title">定位时间：</div>${item.time}</div>
                       ${item.speed ? `<div><div class="point-content__title">速度：</div>${item.speed}</div>` : ''}
                       ${item.bearing ? `<div><div class="point-content__title">方向：</div>${item.bearing}</div>` : ''}
                       ${item.altitude ? `<div><div class="point-content__title">海拔：</div>${item.altitude}</div>` : ''}
                       ${content ? `<div class="point-content__temp">
                            <div class="point-content__title">温度：</div>
                            <div>${content}</div>
                         </div> ` : ''}
                    </div>`;
            },
            ...
            createSimpleMarker(i, iconIndex) {
                ...
                window.$lazyAMapApiLoaderInstance().then(() => {
                    ...
                    window.AMapUI.loadUI(['overlay/SimpleMarker'], SimpleMarker => {
                        ...
                        m.on('mouseover', () => that.openInfoWindow(i));
                    });
                });
            },
            createSvgMarker(i) {
                ...
                window.$lazyAMapApiLoaderInstance().then(() => {
                    ...
                    window.AMapUI.loadUI(['overlay/SvgMarker'], SvgMarker => {
                        ...
                        marker.on('mouseover', () => that.openInfoWindow(i));
                    });
                });
            ...
            exportTrajectory() {
                ...
                let params = {
                    ...
                    startDate: this.formItem.departTime,
                    endDate: this.formItem.arriveTime,
                    carNumber: this.formItem.plateNumber
                };
                this.$http.get('/location-data/car-trajectory/export', {params}).then(res => {
                    if (res.data.path) {
