...
#myTablesInfo {
    ...
    #d-table-left_scroll .el-scrollbar__view, .content-right {
        width: 100%;
        height: 100%;
        position: relative;
    }
        ...
        <div class="container d-table f-w flexDis">
            ...
            <span class="pull-right" v-has="'ccs-warehouse-image-edit'">
              <a @click.stop.prevent="addGraph" class="btn-circle" href="#" v-has="'show'">
                ...
                <el-scrollbar :style="'height:'+ (bodyHeight-20) + 'px'" class="d-table-left_scroll layoutList"
                              tag="div">
                        ...
                        <div style="width:100%; overflow-x:hidden;" v-else>
                            <ul class="show-list">
                                <li :class="{'active':item.backgroundId===currentGraph.backgroundId}" :key="index"
                                    ...
                                    <span class="des-btn_name applyFlex"
                                          :title="item.backgroundName">{{ item.backgroundName }}</span>
                                    <span class="des-btn__position flexDis"
                                          v-show="item.backgroundId===currentGraph.backgroundId">
            ...
            <div class="d-table-right">
                ...
                -->
                <div :style="'height:'+ (bodyHeight+20) + 'px;'" class="d-table-left_scroll" id="d-table-left_scroll"
                     tag="div">
                                    ...
                                    <div class="header">
                                        ...
                                        <div class="header-right">
                                            ...
                                            <div style="font-size: 14px;margin-left: 15px">
                                                <el-tooltip :content="warehouseList.join('，')" placement="right"
                                                            style="width: 200px">
                                                    <span> 仓库编码</span>
                                                </el-tooltip>
                                            </div>
                                            <f-a :class="['alarm-sign', alarmSignClass]" name="alarm"
                                                 v-show="isAlarming"></f-a>
                                            <audio hidden id="alarmMusic" ref="alarmMusic"
                                                   src="@/../static/audio/alarm.mp3"></audio>
                                        </div>
                                    </div>
                                </el-col>
                                <!--
                                ...
                                <el-col :span="10" align="right"
                                    ...
                                    <el-button-group>
                                        ...
                                        <el-button @click="pointConfigFn" v-if='!isPointConfig'
                                                   v-has="'ccs-warehouse-image-edit'" plain="" size="mini">原点设置
                                        </el-button>
                                        <el-button @click="savePointConfigFn" v-if='isPointConfig'
                                                   v-has="'ccs-warehouse-image-edit'" plain="" size="mini">保存原点设置
                                        </el-button>
                                        <el-button @click="cancelPointConfigFn" v-if='isPointConfig'
                                                   v-has="'ccs-warehouse-image-edit'" plain="" size="mini">取消原点设置
                                        </el-button>
                                        <el-button @click="showBigMap" plain="" size="mini">查看大图</el-button>

                                        <el-button @click="unitRight" plain="" size="mini"
                                                   v-has="'ccs-org-devmap-authorized'">单位授权
                                        </el-button>
                                        <!--<el-button plain="" size="mini" v-has="'ccs-org-devmap-authorized'">单位授权</el-button>-->

                                        <el-button @click="doEditPos" plain="" size="mini"
                                                   v-has="'ccs-warehouse-image-edit'" v-if="!editPosition">编辑设备位置
                                        </el-button>
...
<!--                <VueDraggableResizable
                                    ...
                                    :key="v.pointId"
                                >
                                    <h3 class="titleTxt">{{ v.pointName }}{{ v.isActived }}<span
                                ...
                                <div id="svgPart" ref="svgPart" class="svgPart applyFlex imgUrlInfo flexWStatic"
                                    ...
                                    <tmPer :color="item.color" :fontColor="item.fontcolor" :item="item.devDetail"
                                           :key="index" :position="item.position" @goTo="goTo"
                                    ...
                                    <tm :color="item.color" :fontColor="item.fontcolor" :item="item.devDetail"
                                        :key="item.ccsDevId" :position="item.position" @goTo="goTo"
                                        :read-only="true"
                                        :size="12" :tmData="ccsWarehouseImagePointRelationDTOListData" ref="tm-point"
                                        v-for="(item, index) in ccsWarehouseImagePointRelationDTOListData">
                                        {{ item.text }}
                                    </tm>
...
<!--                  <img :src="imgUrl" @mousemove="() => null" style="user-select:none; pointer-events:none; z-index:-1;" :style="{ width : imgWidth + 'px', height : imgHeight + 'px' }" />-->
        ...
        <page-right :css="{'width':'900px','padding':0}" :show="showIndex !== -1" @right-close="resetRightBox">
            <form-part :form-item="currentGraph" :index="showIndex" @refresh="refresh"
                       v-show="showIndex === 1"></form-part>
            <edit-form-part :form-item="currentGraph" :normalIconUrlBase64="normalIconUrlBase64"
                            :offlineIconUrlBase64="offlineIconUrlBase64" :index="showIndex" @refresh="refresh"
                            v-show="showIndex === 3" ref="editForm"/>
                        ...
                        */
                        ...
                        devDetail: {...m,
                            indoorPositionSceneDTO: {
                                ...this.currentGraph.indoorPositionSceneDTO,
                                normalIconUrlBase64: this.normalIconUrlBase64,
                                offlineIconUrlBase64: this.offlineIconUrlBase64
                            }
                        }
                    };
