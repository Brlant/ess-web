...
export default {
    ...
    watch: {
        ...
        // yxh 新增 form 用于传递 基本信息
        form( val ){
            // console.error( val, 8891 ) ;
        },
    },
    methods: {
        doClose() {
        ...
        onSubmit(formName) {
            this.$refs[formName].validate((valid) => {
                    ...
                    // this.point.id = this.form.id ;
                    this.point.belongObjectId = this.form.id ;

                    // 对应的点位类型：1：库区；2：车辆；3：冷柜
                    this.point.pointType    =   +this.form.monitordevType === 1 ?  // 如果是 车辆类型 1 
                                                2 
                                                :
                                                +this.form.monitordevType === 2 ?  // 如果是 冷柜类型 2 
                                                3 : '' ;

                    // console.error( this.point, 18 ) ;
