  ...
  <div class="order-page">
    ...
    <div class="order-list" style="margin-top: 20px">
      <el-row class="order-list-header">
        ...
        <el-col :span="4">点位名称/设备编码</el-col>
...
<script>
    ...
    export default {
        ...
        mounted() {
            // 解决其它界面跳转异步数据相互覆盖问题
            if( !Object.keys(this.$route.query).length ){ // 如果没有携带参数
              this.queryList(1); // 初始化列表数据
            }
        ...
        methods: {
            // initFn(){
            //   this.getMonitorRelationListFn() ; // 请求列表接口数据
            // },
            // getMonitorRelationListFn(){
            //   let params = {
            //     pageNo: this.pager.currentPage,
            //     pageSize: this.pager.pageSize,
            //     ...this.filters,
            //     activeFlag : this.filters.status,
            //   } ;

            //   MonitoringObjGroup.getMonitorRelationList( params )
            //   .then( d => {
            //     console.error( d ) ;
            //   } )
            //   .catch( err => {
            ...
            queryList(pageNo) {
                // yxh 之前逻辑
                const http = MonitoringObjGroup.getMonitorRelationList;
                // const http = MonitoringObjGroup.getMonitorRelationList;
