...
export function init(Vue) {
    ...
    Vue.filter('adcode2city', function (code) {
        let area = Area.findCityByAdcode(code, Area.adcodes)
        return area ? area.label : ''
    });
    Vue.filter('echartTooltipDate', function (dateTime) {
        if (!dateTime) return '';
    ...
    Vue.filter('formatAuditStatus', function (val) {
    ...
    Vue.prototype.$formatDevType = function (item) {
        return +item.devType === 4 ? // 4 为湿度计    非 4 为温度计取 temperature 字段
                2 : 
                1 ;
    ...
    //跳转到历史数据查询
    Vue.prototype.$turnDevDate = function (item, type, isHistory) {
        ...
        this.$router.push({path: '/monitoring/point-dev/temp', query: {id, type, isHistory}});
    };

    //跳转到点位的历史数据查询
    Vue.prototype.$turnPointDevDate = function (item, type) {
        let id = item.pointId;
        this.$router.push({path: '/monitoring/point-dev/temp', query: {id, type}});
    };
